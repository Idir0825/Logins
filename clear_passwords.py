...
from constants import CLEAR_PASSWORDS_PATH
from db_handler import load_db, save_db

if os.path.exists(CLEAR_PASSWORDS_PATH):
    LOGINS = load_db(CLEAR_PASSWORDS_PATH)
else:
    LOGINS = {}


def register_clear(user=None, pw=None):
    ...
    LOGINS[user_id] = password_two
    save_db(CLEAR_PASSWORDS_PATH, LOGINS)
    print(f"New user {user_id} saved successfully !")


def login_clear(user=None, pw=None):
    user_exists = False
    ...
    while password not in LOGINS[user_id]:
        tries -= 1
        if tries == 0:
...
if __name__ == '__main__':
    ...
    print(f"All users can be found in folder : {CLEAR_PASSWORDS_PATH}")
